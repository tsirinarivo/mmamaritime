...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'pages.logistique' })
  return {
    title: t('heroTitle'),
...
export default async function LogistiquePage() {
  ...
  const t = await getTranslations('pages.logistique')
  ...
  const jsonLd = {
    ...
    '@type': 'Service',
    name: 'Logistique Portuaire Madagascar',
    provider: {
      '@type': 'Organization',
    ...
    description: t('heroSubtitle'),
    url: `https://mmamaritime.com/${locale}/services/logistique`,
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {servicesItems.map((item, i) => (
              <ScrollReveal key={i} delay={i * 0.07}>
                <div className="p-6 border border-cream-300 rounded-sm hover:border-gold/30 transition-colors hover:shadow-sm group h-full">
                  <div className="w-8 h-8 bg-teal/10 rounded-sm flex items-center justify-center text-teal font-sans font-bold text-sm mb-4 group-hover:bg-teal group-hover:text-white transition-colors">
